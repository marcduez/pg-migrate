...
export const getCreateTableAndViewStatements = async (client: Client) => {
  ...
  order by schemaname::text, viewname::text`)
  ...
  const viewColumnCommentsByViewName = new Map<string, string[]>(
    viewColumnCommentRows.reduce(
      (map, { comment, name, schema_name, view_name }) => {
        const schemaAndViewName = `${schema_name}.${view_name}`
        if (!map.has(schemaAndViewName)) {
          map.set(schemaAndViewName, [])
        }
        map
          .get(schemaAndViewName)!
          .push(
    ...
    >(
      (
        ...
        const columnCommentLine = comment
          ? `COMMENT ON COLUMN ${schemaAndTableName}.${name} IS ${comment};`
          : null
        ...
        let item = arr.find(item => item.name === schemaAndTableName)
        if (!item) {
          item = {
        ...
        item.columns.push(columnLine)
        if (columnCommentLine) {
          item.columnComments.push(columnCommentLine)
        }
        return arr
      },
      [],
            ...
            statements: [
              ...
              ...(comment
                ? [
                    `COMMENT ON ${
                      is_materialized ? "MATERIALIZED " : ""
                    }VIEW ${schemaAndViewName} IS ${comment};`,
                  ]
                : []),
              ...(viewColumnCommentsByViewName.get(schemaAndViewName) ?? []),
            ],
          }
        },
      ),
